...
/**
 * Extracted from app/dashboard/services/detail/page.tsx, which previously
 * inlined ~140 lines of detail rendering directly in the route file — the
 * only services surface not living in the feature folder. The route file is
 * now just the `useSearchParams` + `Suspense` wrapper Next.js requires.
 */
...
export const ServiceRequestDetail = ({ serviceId }: { serviceId: string }) => {
    ...
    return (
      <div className="rounded-2xl p-8 text-center" style={{ border: "1px solid var(--border)", backgroundColor: "var(--card)" }}>
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--medium-gray)" }}>
          {error ?? "This request could not be loaded."}
        </p>
        <Link
  ...
  const detailSections: Array<{ label: string; value?: string | number | null }> = [
    { label: "Service type", value: meta.title },
      ...
      >
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 p-6">
          <div className="flex items-start gap-4">
            <span className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl text-3xl" style={{ backgroundColor: tintBg(meta.accent) }}>
              {meta.emoji}
            </span>
        ...
        >
          ...
          {detailSections
            ...
            .map((s) => (
              ...
              >
                <span className="w-28 flex-shrink-0 text-xs font-semibold" style={{ color: "var(--medium-gray)" }}>
                  {s.label}
                </span>
