...
const buildOtpCodePanel = (otp, label = 'Verification code') => {
  const safeOtp = toSafeString(otp) || '------';
  const safeLabel = toSafeString(label) || 'Verification code';
  ...
  <div style="margin: 24px 0; padding: 18px; border-radius: 14px; background: ${EMAIL_STYLES.primarySoft}; border: 1px solid ${EMAIL_STYLES.primarySoftBorder}; text-align: center;">
    <p style="margin: 0 0 8px; color: ${EMAIL_STYLES.mutedText}; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase;">${escapeHtml(safeLabel)}</p>
...
// Carries both reset credentials generated by one request: a short numeric
// code for typing into the mobile/web reset screen, and a long-lived link
// for one-click reset on web (and, once the app ships universal-link
// config, directly into the app on a phone). Both redeem the same
// server-side request and expire together — see password-reset.service.js.
const sendPasswordResetEmail = async ({ to, fullName, resetCode, resetLink, expiresInMs }) => {
  const appName = getAppName();
  const safeName = toSafeString(fullName) || 'there';
  const safeCode = toSafeString(resetCode);
  const safeLink = toSafeString(resetLink) || config.appUrl;
  const expiresInMinutes = Math.max(1, Math.ceil(Number(expiresInMs || 0) / 60000));
...
Reset it with one tap:
${safeLink}

Or enter this code on the reset screen:

${safeCode}

This expires in ${expiresInMinutes} minute${expiresInMinutes === 1 ? '' : 's'}.
...
${appName} Team
  ...
  const html = buildEmailLayout({
    ...
    bodyHtml: `
      <p style="margin: 0;">Hi <strong>${escapeHtml(safeName)}</strong>,</p>
      <p style="margin: 14px 0 0;">You (or someone using your email) asked to reset your ${escapeHtml(appName)} password. Tap the button below, or enter the code on the reset screen.</p>
      ${buildOtpCodePanel(safeCode, 'Password reset code')}
      <p style="margin: 0;">This expires in <strong>${expiresInMinutes} minute${expiresInMinutes === 1 ? '' : 's'}</strong>.</p>
      <p style="margin: 14px 0 0; color: ${EMAIL_STYLES.mutedText}; font-size: 12px; word-break: break-all;">Button not working? Paste this link instead: ${escapeHtml(safeLink)}</p>
