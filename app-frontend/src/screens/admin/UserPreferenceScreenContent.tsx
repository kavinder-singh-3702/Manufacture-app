...
import { CampaignThemeKey, preferenceService, PreferenceSummary, PersonalizedOffer } from "../../services/preference.service";
...
import { CAMPAIGN_THEME_OPTIONS, getCampaignThemeLabel } from "../dashboard/components/campaignTheme";
...
export const UserPreferenceScreen = () => {
  ...
  const [form, setForm] = useState({
    ...
    title: "",
    message: "",
    themeKey: "campaignFocus" as CampaignThemeKey,
    newPrice: "",
    oldPrice: "",
    minOrderValue: "",
    expiresAt: "",
  ...
  const handleCreateOffer = useCallback(async () => {
    ...
    try {
      await preferenceService.createCampaign(userId, {
        ...
        creative: { themeKey: form.themeKey },
        ...
        <View style={{ gap: spacing.md, marginTop: spacing.md }}>
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, padding: spacing.md }]}>
            ...
            <View style={{ gap: spacing.sm }}>
              ...
              <View style={{ gap: spacing.xs }}>
                <Text style={[styles.meta, { color: colors.textMuted }]}>Theme style</Text>
                <View style={styles.themeStyleRow}>
                  {CAMPAIGN_THEME_OPTIONS.map((option) => {
                    const isActive = form.themeKey === option.key;
                    return (
                      <TouchableOpacity
                        key={option.key}
                        onPress={() => setForm((prev) => ({ ...prev, themeKey: option.key }))}
                        activeOpacity={0.86}
                        style={[
                          styles.themeChip,
                          {
                      ...
                      >
                        <LinearGradient
                          colors={nativeGradients[option.gradientKey]}
                          start={{ x: 0, y: 0 }}
                          end={{ x: 1, y: 1 }}
                          style={styles.themeChipSwatch}
                        />
                        <View style={{ flex: 1 }}>
                          <Text style={{ color: isActive ? colors.primary : colors.text, fontWeight: "700", fontSize: 12 }}>
                            {option.label}
                          </Text>
                  ...
                  <View key={offer.id} style={[styles.row, { justifyContent: "space-between" }]}>
                    <View>
                      ...
                      <Text style={[styles.meta, { color: colors.textSecondary }]}>
                        Theme: {getCampaignThemeLabel(offer.creative?.themeKey)}
                      </Text>
                      {offer.expiresAt && (
...
const styles = StyleSheet.create({
  ...
  themeStyleRow: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  themeChip: {
    width: "100%",
    minHeight: 58,
    borderWidth: 1,
